...
KindEditor.lang({
	...
	'image.remoteImage' : '缃戠粶锲剧墖',
	'image.localImage' : '链湴涓娄紶',
	'image.remoteUrl' : '锲剧墖鍦板潃',
	'image.localUrl' : '涓娄紶鏂囦欢',
	'image.size' : '锲剧墖澶у皬',
	'image.width' : '瀹?,
	'image.height' : '楂?,
	...
	'image.upload' : '娴忚...',
	'image.viewServer' : '锲剧墖绌洪棿',
	'multiimage.uploadDesc' : '鍏佽鐢ㄦ埛鍚屾椂涓娄紶<%=uploadLimit%>寮犲浘鐗囷紝鍗曞紶锲剧墖瀹归噺涓嶈秴杩?%=sizeLimit%>',
	'multiimage.startUpload' : '寮€濮嬩笂浼?,
	'multiimage.clearAll' : '鍏ㄩ儴娓呯┖',
	'multiimage.insertAll' : '鍏ㄩ儴鎻掑叆',
	'multiimage.queueLimitExceeded' : '鏂囦欢鏁伴噺瓒呰绷闄愬埗銆?,
	'multiimage.fileExceedsSizeLimit' : '鏂囦欢澶у皬瓒呰绷闄愬埗銆?,
	'multiimage.zeroByteFile' : '镞犳硶涓娄紶绌烘枃浠躲€?,
	'multiimage.invalidFiletype' : '鏂囦欢绫诲瀷涓嶆纭€?,
	'multiimage.unknownError' : '鍙戠敓寮傚父锛屾棤娉曚笂浼犮€?,
	'multiimage.pending' : '绛夊緟涓娄紶',
	'multiimage.uploadError' : '涓娄紶澶辫触',
	...
	'baidumap.search' : '鎼灭储',
	'baidumap.insertDynamicMap' : '鎻掑叆锷ㄦ€佸湴锲?,
	'anchor.name' : '阌氱偣鍚岖О',
		...
		edit : function() {
			...
			self.plugin.imageDialog({
				...
				clickFn : function(url, title, width, height, border, align) {
					...
					// Bugfix: [Firefox] 涓娄紶锲剧墖鍚庯紝镐绘槸鍑虹幇姝ｅ湪锷犺浇镄勬牱寮忥紝闇€瑕佸欢杩熸墽琛宧ideDialog
					setTimeout(function() {
						self.hideDialog().focus();
					}, 0);
				}
	...
	self.plugin.table = {
		...
		rowmerge : function() {
			var table = self.plugin.getSelectedTable()[0],
				row = self.plugin.getSelectedRow()[0],
				cell = self.plugin.getSelectedCell()[0],
				rowIndex = row.rowIndex, // 褰揿墠琛岀殑index
			...
			var cellIndex = _getCellIndex(table, row, cell); // 涓嬩竴琛屽崟鍏冩牸镄刬ndex
			if (nextRow.cells.length <= cellIndex) {
				return;
			}
			var nextCell = nextRow.cells[cellIndex]; // 涓嬩竴琛屽崟鍏冩牸
			// 涓娄笅琛岀殑colspan涓崭竴镊存椂涓嶈兘鍚埚苟
